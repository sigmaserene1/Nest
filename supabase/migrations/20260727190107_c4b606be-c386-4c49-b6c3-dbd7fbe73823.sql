...
CREATE POLICY "roommates readable"
ON public.roommates
FOR SELECT
TO anon, authenticated
USING (true);

CREATE POLICY "roommates insertable"
ON public.roommates
FOR INSERT
TO anon, authenticated
WITH CHECK (true);

CREATE POLICY "roommates deletable"
ON public.roommates
FOR DELETE
TO anon, authenticated
USING (true);
...
CREATE INDEX payment_requests_to_idx
ON public.payment_requests (to_wallet);

CREATE INDEX payment_requests_from_idx
ON public.payment_requests (from_wallet);
...
CREATE POLICY "requests readable"
ON public.payment_requests
FOR SELECT
TO anon, authenticated
USING (true);

CREATE POLICY "requests insertable"
ON public.payment_requests
FOR INSERT
TO anon, authenticated
WITH CHECK (true);

CREATE POLICY "requests updatable"
ON public.payment_requests
FOR UPDATE
TO anon, authenticated
USING (true)
...
CREATE INDEX transactions_from_idx
ON public.transactions (from_wallet);

CREATE INDEX transactions_to_idx
ON public.transactions (to_wallet);
...
ADD TABLE public.roommates;

ALTER PUBLICATION supabase_realtime
ADD TABLE public.payment_requests;
