...
import { useMembers, useExpenses } from "@/lib/chain/nest-chain";
...
function Analytics() {
  ...
  const byCat = expenses.reduce<Record<string, number>>(
    (a, e) => ((a[e.category] = (a[e.category] ?? 0) + e.amount), a),
    {},
  );
  const catEntries = Object.entries(byCat).sort((a, b) => b[1] - a[1]);

  const byPayer = expenses.reduce<Record<string, number>>(
    (a, e) => ((a[e.payerId] = (a[e.payerId] ?? 0) + e.amount), a),
    {},
  );
    ...
    <AppShell
      greeting={
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight sm:text-[28px]">Insights</h1>
        </div>
      }
    >
          ...
          {[
            ...
            return (
              <div key={b.m} className="flex flex-1 flex-col items-center gap-2">
                ...
                <div
                  className={`text-[11px] font-semibold ${isLast ? "text-brand" : "text-muted-foreground"}`}
                >
                  {b.m}
                </div>
              </div>
...
function MemberAvatar32({ id }: { id: string }) {
  ...
  return (
    <span
      className="inline-grid h-8 w-8 place-items-center rounded-full text-[11px] font-semibold text-white"
